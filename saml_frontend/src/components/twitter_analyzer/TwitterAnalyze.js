...
class TweetsAnalyze extends Component {
    state = {
        keyword: '',
        count: 10,
        tweets: [],
    ...
    onSubmit = async (e) => {
        ...
        try {
            ...
            const resKeyword = await axios.post("http://localhost:8000/api/tweets/", key);
    ...
    render() {
        return (
            <div className="container jumbotron mt-3 bg-light text-center">
                <h5 className="display-4 text-uppercase">Twitter Tweets Analysis</h5>
                <hr className="my-4" />
                <form onSubmit={this.onSubmit} className="was-validated">
                    ...
                    <div className="form-group">
                        <label htmlFor="Count">Total no. of tweets: </label>
                        <input 
                            type="number"
                            className="form-control"
                            id="Count"
